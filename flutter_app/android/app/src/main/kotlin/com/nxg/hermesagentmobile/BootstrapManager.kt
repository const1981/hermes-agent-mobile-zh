...
class BootstrapManager(
     ...
     */
    /**
     * Expose the `hermes` CLI in the guest PATH.
     *
     * bootstrap installs Hermes into a venv at /root/hermes-agent/venv, so the
     * bare `hermes` command isn't on PATH for interactive terminal users
     * (they saw "hermes: command not found"). Create a wrapper at
     * /usr/local/bin/hermes that activates the venv and execs hermes.
     * Idempotent — safe to call on every setup.
     */
    fun ensureHermesCli() {
        try {
            val hermesAgent = File("$rootfsDir/root/hermes-agent")
            if (!hermesAgent.exists()) return
            val binDir = File("$rootfsDir/usr/local/bin")
            binDir.mkdirs()
            val wrapper = File(binDir, "hermes")
            val script = "#!/bin/sh\n" +
                "# Auto-generated by Hermes Agent bootstrap - exposes the hermes CLI.\n" +
            ...
            // Non-fatal - hermes CLI simply won't be on PATH.
        }
    }
    ...
    private fun configureRootfs() {
        ...
        // 9. Expose `hermes` CLI in PATH so terminal users can run `hermes`.
        ensureHermesCli()
    }
