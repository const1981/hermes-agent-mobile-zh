...
凭据从 项目文档/cf_upload.env 读取（QINIU_AK / QINIU_SK / bucket=hermesmobile / domain）。
"""
...
AK = ENV.get("QINIU_AK", "")
SK = ENV.get("QINIU_SK", "")
BUCKET = ENV.get("QINIU_BUCKET", "hermesmobile")
DOMAIN = ENV.get("QINIU_DOMAIN", "tiano67r8.hd-bkt.clouddn.com")
...
def main():
    ...
    q = Auth(AK, SK)
    apk_key = f"hermes-agent-mobile-v{version}.apk"
    # 1) 上传 APK
    ...
    print(f"[2/2] 上传 version.json -> {BUCKET}/version.json")
    token2 = q.upload_token(BUCKET, "version.json", 3600)
    ret2, info2 = put_file(token2, "version.json", None,
                            data=version_json)
    if info2.status_code != 200:
        sys.exit(f"version.json 上传失败: {info2.status_code} {ret2}")
    print("     OK:", ret2)
    print(f"\n完成！更新源: http://{DOMAIN}/version.json")
